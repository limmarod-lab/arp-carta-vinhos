...
async function putWines(env,record){return await binFetch(env,env.WINES_BIN_ID,'PUT',{...record,updatedAt:new Date().toISOString()})}
...
export async function onRequest(context){
  ...
  try{
    ...
    if(path==='/wines'&&request.method==='GET'){
      const r=await getWines(env);return json({wines:r.wines||[]},200,request);
    }
    if(path==='/wines'&&request.method==='PUT'){
      ...
      const r=await putWines(env,{version:1,wines:body.wines});return json({wines:r.wines||body.wines},200,request);
    }
    if(path==='/wines'&&request.method==='POST'){
      ...
      const r=await getWines(env);let wines=Array.isArray(r.wines)?r.wines:[];const idx=wines.findIndex(x=>String(x.id)===String(body.wine.id));if(idx>=0)wines[idx]=body.wine;else wines.push(body.wine);wines.sort((a,b)=>String(a.name).localeCompare(String(b.name),'pt-BR'));const saved=await putWines(env,{version:1,wines});return json({wines:saved.wines||wines},200,request);
    }
    const wm=path.match(/^\/wines\/(.+)$/);
    if(wm&&request.method==='DELETE'){
      const p=await auth(request,env,['admin']);if(!p)return json({error:'Acesso restrito ao administrador.'},403,request);
      const id=decodeURIComponent(wm[1]);const r=await getWines(env);const wines=(r.wines||[]).filter(x=>String(x.id)!==String(id));const saved=await putWines(env,{version:1,wines});return json({wines:saved.wines||wines},200,request);
    }
