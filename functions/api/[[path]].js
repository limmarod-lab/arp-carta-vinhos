...
async function binFetch(env,id,method='GET',body){
  ...
  const r=await fetch(`${JSONBIN_ROOT}/${id}${method==='GET'?'/latest':''}`,{
    method,
    headers:{'Content-Type':'application/json','X-Master-Key':env.JSONBIN_MASTER_KEY,'X-Bin-Versioning':'false'},
    body:body?JSON.stringify(body):undefined
  });
  const text=await r.text();
  let data;try{data=JSON.parse(text)}catch{data={message:text}}
  if(!r.ok)throw Error(data.message||`JSONBin ${r.status}`);
...
async function putWines(env,record){
  return binFetch(env,env.WINES_BIN_ID,'PUT',{
    version:6,
